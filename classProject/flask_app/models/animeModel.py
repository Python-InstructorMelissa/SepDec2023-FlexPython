...
class Anime:
    ...
    @classmethod
    def save(cls, data):
        query = "INSERT INTO anime (name, show, alignment, power, img, user_id) VALUES (%(name)s, %(show)s, %(alignment)s, %(power)s, %(img)s, %(user_id)s);"
    ...
    def update(cls, formData):
        ...
        query = """
        UPDATE anime
        SET name=%(name)s, show=%(show)s, alignment=%(alignment)s, power=%(power)s
        WHERE id = %(id)s;
        """
